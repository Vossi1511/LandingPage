...
import { Mail, Plane, Cpu, Flag, GraduationCap, CarFront, MessageCircle } from 'lucide-react';
...
const socialLinks = [
  { icon: Mail, label: 'Email', href: 'mailto:hello@example.com', color: 'hover:bg-red-500/20 hover:border-red-500/40 hover:text-red-300' },
...
const alternateNames = ['vossi1511', 'vxssi', 'vxssi_', 'vxssi1511'];
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
              <Badge variant="outline" className="bg-blue-500/20 text-blue-300 border-blue-500/40 flex items-center h-7">
                <GraduationCap className="w-3.5 h-3.5 mr-1" />
                Student
              </Badge>
              <Badge variant="outline" className="bg-slate-700/50 text-slate-200 border-slate-600 flex items-center h-7">
                <span className="text-lg mr-1">🇩🇪</span> Germany
              </Badge>
            </div>
            <p className="text-slate-400 mb-4">
              aka {alternateNames.map((name, i) => (
                <span key={name}>
                  <span className="text-slate-300 font-mono">{name}</span>
                  {i < alternateNames.length - 1 && <span className="text-slate-600"> • </span>}
                </span>
              ))}
            </p>
          </motion.div>
            ...
            >
              <div className="bg-slate-900/50 backdrop-blur-xl rounded-lg border border-slate-700/50 overflow-hidden shadow-2xl">
                <div className="bg-slate-800/80 px-4 py-3 flex items-center gap-2 border-b border-slate-700/50">
                  ...
                  <span className="ml-2 text-slate-400 text-sm">~/setup</span>
                </div>
                <div className="p-6 font-mono text-xs sm:text-sm overflow-x-auto">
                  <div className="text-green-400">
                    <span className="text-slate-500">$</span> <TerminalText text="neofetch" delay={1500} speed={80} />
                  </div>
                  {showNeofetch && (
                    <>
...
################  ################`}
                        ...
                        <div className="text-slate-300 space-y-1">
                          <div><span className="text-blue-400">vossi</span>@<span className="text-blue-400">setup</span></div>
                          <div className="text-slate-600">-----------------</div>
                          <div><span className="text-slate-500">OS:</span> Windows 11</div>
                          <div><span className="text-slate-500">Also:</span> Fedora KDE Plasma</div>
                          <div><span className="text-slate-500">Server:</span> Debian headless</div>
                          <div><span className="text-slate-500">CPU:</span> AMD Ryzen 5 5600X</div>
                          <div><span className="text-slate-500">Memory:</span> 32 GB DDR4 3600 MT/s</div>
                          <div><span className="text-slate-500">GPU:</span> NVIDIA RTX 4060 Ti 16GB</div>
                        </div>
                      </motion.div>
                    </>
                  )}
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">- T.G.</p>
                </div>
              </div>
            </motion.div>
        ...
        >
          <p className="font-mono">
            Made with <span className="text-red-500 animate-pulse">♥</span> using React & Motion
          </p>
        </motion.div>
      </div>
    </div>
